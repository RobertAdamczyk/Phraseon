...
struct TagLayout: Layout {
    ...
    func sizeThatFits(proposal: ProposedViewSize, subviews: Subviews, cache: inout ()) -> CGSize {
        ...
        var height: CGFloat = 0  // Variable 'height' was never used; consider replacing with '_' or removing it
        let rows = generateRows(maxWidth, proposal, subviews)

        for (index, row) in rows.enumerated() {  // Immutable value 'index' was never used; consider...
...
extension [LayoutSubviews.Element] {
    fileprivate func maxHeight(_ proposal: ProposedViewSize) -> CGFloat {
